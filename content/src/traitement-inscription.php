...
if(isset($_POST['submit']))
{
   if(!empty($_POST['nom']) AND !empty($_POST['prenom']) AND !empty($_POST['pseudo']) AND !empty($_POST['email']) AND !empty($_POST['password']) AND !empty($_POST['password2']) AND $_POST['password']==$_POST['password2'])
   {
      ...
      if(preg_match($password_regex, $_POST['password']))
      {
          ...
          // Vérifier si le champ utilisateur 'mail' n'existe pas déjà dans la BDD
            ...
            $user = $stmt->fetch();
                ...
                $insertUser = 'INSERT INTO users(mdp_user, nom_user, prenom_user, pseudo_user, mail_user, avatar_id, id_role) VALUES (:mdp_user, :nom_user, :prenom_user, :pseudo_user, :mail_user, :avatar_id, :id_role)';
                ...
                $user["pseudo_user"] = $_POST['pseudo'] ;
                $_SESSION['msg_success_register'] = 'Bienvenue ' . $user["pseudo_user"].', vous êtes enregistré.' . '<br>' . '<a class="underline" href="/metromax/content/pages/connexion.php">Cliquez pour vous connecter</a>';
      ...
      // Erreurs Regex
        ...
        echo '<script>alert("Votre mot de passe ne respecte pas les conditions requises !")</script>';
      }
